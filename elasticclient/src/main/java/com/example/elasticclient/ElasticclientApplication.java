...
public class ElasticclientApplication {
	...
	public static void main(String[] args) {
		...
		new ElasticclientApplication().TestSearch();
	}
	...
	public void TestSearch() {
		...
		connector.Connect();
		try {
			connector.getInAppByDate("2026-01-01", "2026-02-05", "US", "", "", null);
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public void TestIap() {
		...
		try {
			for (int i = 0; i < 2; i++) {
				int index = i + 300;
				Date d = DateFormat.getDateInstance(DateFormat.SHORT).parse("01/07/26 00:00:00");
				int r = new Random().nextInt(100);
				d = new Date(d.getTime() + (r % 7) * 86400000L);
				IapLogItem iapLogItem = new IapLogItem("game1", "iap", "user" + i,
						platforms[r % 2], "1.0.0", countries[r % 5], i * 2 + 1, 10, d, d, "product_" + (i % 10),
						placements[r % 3], "", "transaction_" + i, "order_" + i,
						"completed",
